...
public final class DurationShorthand {
    ...
    public static DurationShorthand parse(String text) {
        ...
        if (!matcher.matches()) {
            throw new IllegalArgumentException(
                    "invalid duration shorthand: " + text + ", expected [0-9]+[mhdw]");
        }
    ...
    public void validate(String path, List<ValidatorError> errors) {
        if (unit == null) {
            errors.add(new ValidatorError("'" + path + ".unit' is required").withPropertyPath(path + ".unit"));
        } else {
            Rules.oneOf(path + ".unit", unit, List.of(Unit.values()), errors);
        }
        Rules.gte(path + ".value", value, 0, errors);
    }
}
